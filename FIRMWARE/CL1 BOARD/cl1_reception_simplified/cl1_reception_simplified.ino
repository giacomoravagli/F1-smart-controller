...
unsigned long ID = 0;
unsigned char tx_buffer[0x02] = {0x00, 0x00}; // payload = 2 bytes

/* Variables name for the Wireless transmission */
const static uint8_t RADIO_ID = 0;       // Our radio's id.  The transmitter will send to this id.
const static uint8_t PIN_RADIO_CE = 6;   // PE0 in the eval board --> to be changed to 6 = PC3 in final board
...
void loop()
{
    while (_radio.hasData())
    {
        ...
        tx_buffer[1] = (uint8_t)(_radioData.AccelerationTx);
...
void send_data_to_CAN_bus()
{
  ...
  CAN_Transmit(0x0001,  CAN_Id_Extended, CAN_RTR_Data, 2, tx_buffer);  /* ID: 0b0000 0100 1001 0110 */
  delay(1);
}
